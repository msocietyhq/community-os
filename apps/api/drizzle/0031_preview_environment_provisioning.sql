CREATE TABLE "project_bootstrap_tokens" (
	...
	CONSTRAINT "project_bootstrap_tokens_token_hash_unique" UNIQUE("token_hash")
);
...
ALTER TABLE "project_bootstrap_tokens" ADD CONSTRAINT "project_bootstrap_tokens_project_id_projects_id_fk" FOREIGN KEY ("project_id") REFERENCES "public"."projects"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "project_bootstrap_tokens" ADD CONSTRAINT "project_bootstrap_tokens_created_by_user_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."user"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
-- ADR-008. dev_environments.owner_id is NOT NULL and FKs to user.id, so this
