  ...
  const eventsTools = {
    list_events: tool({
      ...
      execute: async ({ status, limit }) => {
        console.log("[events-agent:list_events]", { status, limit });
        const { data, error } = await ctx.api.api.v1.events.get({
          query: { status, page: 1, limit: limit ?? 5 },
        });
        if (error) { console.error("[events-agent:list_events] error:", error.status, error.value); return { status: error.status, value: error.value }; }
        console.log("[events-agent:list_events] returned", Array.isArray(data) ? data.length : 1, "item(s)");
        return data;
      },
    }),
    ...
    get_event: tool({
      ...
      execute: async ({ event_id }) => {
        console.log("[events-agent:get_event]", { event_id });
        const { data, error } = await ctx.api.api.v1
          .events({ id: event_id })
          .get();
        if (error) { console.error("[events-agent:get_event] error:", error.status, error.value); return { status: error.status, value: error.value }; }
        return data;
      },
    }),
    ...
    rsvp_event: tool({
      ...
      execute: async ({ event_id, status }) => {
        ...
        if (error) { console.error("[events-agent:rsvp_event] error:", error.status, error.value); return { status: error.status, value: error.value }; }
        return data;
      },
    }),
    ...
    create_event: tool({
      ...
      execute: async ({
        ...
        console.log("[events-agent:create_event]", { title, event_type, starts_at, ends_at, is_online });
        ...
        if (error) { console.error("[events-agent:create_event] error:", error.status, error.value); return { status: error.status, value: error.value }; }
        return data;
      },
    }),
    ...
    update_event: tool({
      ...
      execute: async ({
        ...
        if (error) { console.error("[events-agent:update_event] error:", error.status, error.value); return { status: error.status, value: error.value }; }
        return data;
      },
    }),
    ...
    delete_event: tool({
      ...
      execute: async ({ event_id }) => {
        ...
        const { data, error } = await ctx.api.api.v1
          .events({ id: event_id })
          .delete();
        if (error) { console.error("[events-agent:delete_event] error:", error.status, error.value); return { status: error.status, value: error.value }; }
        return data;
      },
    }),
  };
