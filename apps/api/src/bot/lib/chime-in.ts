...
/**
 * A notice about the bot's own state, rather than an answer to anything.
 *
 * "Your profile is not set up yet", "I'm having trouble authenticating you",
 * "I'm being rate-limited", "Sorry, I encountered an error" — each of these was
 * reachable on the chime-in path, and each would have gone to someone who asked
 * the room a question and never asked the bot anything.
 *
 * "Your profile is not set up yet" is the one that would have bitten hardest:
 * in a 500-member group plenty of people have no profile, so their questions to
 * each other would have been answered with a demand that they run /profile.
 *
 * Uninvited, none of them are sent. Addressed, all of them are.
 */
export function asideIfPermitted(
  text: string,
  chimingIn: boolean,
): string | null {
  return chimingIn ? null : text;
}

/**
 * The model's own answer for the turn.
 *
 * Uninvited, empty text means silence: a turn that ended on a tool call or
 * spent its step budget has nothing to say, and the caller's fallback chain
 * would otherwise put a generic apology into a group that never asked.
 * Addressed, the text is returned untouched and the caller keeps its own
 * fallback behaviour.
 */
export function answerIfPermitted(
  text: string | null | undefined,
  chimingIn: boolean,
): string | null {
  if (!chimingIn) return text ?? null;
