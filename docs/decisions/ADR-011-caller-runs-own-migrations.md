...
- **The reusable `preview.yml` workflow exposes `database_url` as a job output** from its `ensure` job, so a caller repo adds one more job — `needs: preview`, running its own migration command with `DATABASE_URL: ${{ needs.preview.outputs.database_url }}` — only if it has migrations to run. Getting this output to actually come through took two rounds: GitHub Actions silently drops ("Skip output ... since it may contain secret") any output set by a step whose script references `secrets.*` at all, regardless of what the output value itself contains — not, as first assumed, because of an explicit `::add-mask::` call. The `ensure` job now writes the API response to a file in the step that touches `secrets.COMMUNITY_OS_CI_TOKEN`, then reads it back and sets the output in a separate step with no `secrets.` reference of its own.
