...
- **The reusable `preview.yml` workflow exposes `database_url` as a job output** from its `ensure` job, so a caller repo adds one more job — `needs: preview`, running its own migration command with `DATABASE_URL: ${{ needs.preview.outputs.database_url }}` — only if it has migrations to run. It's deliberately left unmasked: GitHub Actions silently drops (with a "may contain secret" warning) any job output whose value was passed through `::add-mask::`, so masking and exporting a value as an output are mutually exclusive — masking it broke the whole feature the first time around.
