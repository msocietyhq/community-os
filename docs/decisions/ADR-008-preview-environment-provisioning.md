...
- **The reusable GitHub Actions workflow lives once, in this repo** (`.github/workflows/preview-db.yml`, `workflow_call`), calling `ci/ensure`/`ci/teardown`. A new project's own workflow file is ~5 lines (`uses: msocietyhq/community-os/.github/workflows/preview-db.yml@main` plus the one inherited secret) — see that file's header comment for the exact snippet.
